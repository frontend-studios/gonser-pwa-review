<template>
  <div data-testid="checkout-address" class="px-4 @md:px-0 pb-6">
    <div v-if="isLoading" class="hidden @sm:flex @sm:flex-row @sm:items-center justify-between">
    ...
    <div>
      <div
        v-if="isShipping || !shippingAsBilling"
      ...
      <div class="mt-2">
        ...
        <template v-if="isBilling">
          ...
          <template v-else-if="hasCheckoutAddress && !showSameAsShippingText && !shippingAsBilling">
...
import type { AddressContainerProps } from '~/components/AddressContainer/types';
  ...
  () => {
    if (isBilling) {
      if (!shippingAsBilling.value) {
        // Checkbox unchecked: Zeige Billing-Formular
        showNewForm.value = true;
        editing.value = false;
      } else {
        // Checkbox checked: Verstecke Billing-Formular
        showNewForm.value = false;
        editing.value = false;
      }
    }
