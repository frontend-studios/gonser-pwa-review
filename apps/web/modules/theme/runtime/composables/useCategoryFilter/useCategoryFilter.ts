...
export const useCategoryFilter = (to?: RouteLocationNormalizedGeneric): UseCategoryFiltersResponse => {
   ...
   ***/
  const updateQuery = async (parameter?: object) => {
    const query = { ...getFiltersDataFromUrl(), ...parameter };
    const entries = Object.entries(query);
    const updateQuery: { [key: string]: string } = {};
    entries.forEach(([key, value]) => {
      if (value !== null && value !== undefined) {
        updateQuery[key] = (value as string | number | boolean).toString();
      }
    });

    if (import.meta.client) {
      await navigateTo({ query: updateQuery });
    }
  };
   ...
   * ```
   */
  const updateFilters = async (filters: Filters): Promise<void> => {
    const currentFilters = getFiltersFromUrl();
    const mergedFilters = mergeFilters(currentFilters, filters);
    const filtersIds = getFiltersToUpdate(mergedFilters);

    if (filtersIds) {
      await updateQuery({ facets: filtersIds });
    } else {
      await updateQuery({ facets: null });
    }
  };
   ...
   * @example
   * ``` ts
   * await updatePrices('1', '1');
   * ```
   */
  const updatePrices = async (priceMin: string, priceMax: string): Promise<void> => {
    if (priceMin.length > 0 && priceMax.length === 0) {
      await updateQuery({ priceMin: priceMin });
    } else if (priceMax.length > 0 && priceMin.length === 0) {
      await updateQuery({ priceMax: priceMax });
    } else if (priceMax.length > 0 && priceMin.length > 0) {
      await updateQuery({ priceMin: priceMin, priceMax: priceMax });
    } else {
      await updateQuery({ priceMin: null, priceMax: null });
    }
  };
  ...
  const checkFiltersInURL = (): void => {
    ...
    if (facetsFromUrl.facets) {
      ...
      const updatedFacets = facets.filter((facet: string) => {
