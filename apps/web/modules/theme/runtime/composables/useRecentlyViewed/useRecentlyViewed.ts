...
export const useRecentlyViewed = (): UseRecentlyViewedReturn => {
  ...
  const fetchProducts = async (): Promise<void> => {
    ...
    // Skip re-fetch if the stored item IDs are identical to the last successful fetch
    const currentIds = storedItems.map((i) => `${i.itemId}:${i.variationId}`).join(',');
    if (currentIds === state.value.loadedItemIds && state.value.items.length > 0) {
      return;
    }
    ...
    try {
      ...
      if (validProducts.length < storedItems.length) {
        ...
        // Record which IDs were loaded so subsequent mounts can skip re-fetching
        state.value.loadedItemIds = currentIds;
      }
    } catch (error) {
