        ...
        <div class="formular lg:col-span-2">
          <div class="p-5 contact-form bg-white rounded-lg shadow">
            ...
            >
              ...
              <div class="mb-5 font-bold text-base">{{ t('theme.ContactPage.subjectHeadline') }}</div>

              <!-- Accordion: Um eine Bestellung -->
              <div class="mb-4">
                <div class="accordion-card border rounded shadow-sm" data-accordion="order">
                  <div class="accordion-header">
                    <h2 class="mb-0 text-base">
                      <button
                        type="button"
                        class="accordion-btn w-full text-left font-bold px-4 py-3 hover:bg-gray-50 transition-colors flex items-center justify-between text-[#151515] relative"
                  ...
                  <div v-show="activeAccordion === 'order'" class="accordion-content p-4">
                    <!-- Unteroptionen -->
                    <div class="space-y-2">
                      <div v-for="(option, index) in orderOptions" :key="index" class="sub-accordion border-0 rounded">
                        ...
                        <div v-show="activeSubAccordion === 'order-' + index" class="sub-accordion-content p-4">
              ...
              <div class="mb-4">
                <div class="accordion-card border rounded shadow-sm" data-accordion="item">
                  <div class="accordion-header">
                    <h2 class="mb-0 text-base">
                      <button
                        type="button"
                  ...
                  <div v-show="activeAccordion === 'item'" class="accordion-content p-4">
                    <div class="space-y-2">
                      <div v-for="(option, index) in itemOptions" :key="index" class="sub-accordion border-0 rounded">
                        <div class="sub-accordion-header p-0">
                          <button
                            type="button"
                            class="sub-accordion-btn w-full text-left px-2 py-3 flex items-center justify-between hover:bg-gray-50 transition-colors relative"
                        ...
                        <div v-show="activeSubAccordion === 'item-' + index" class="sub-accordion-content p-4">
                          <p class="text-sm text-gray-600 mb-4">
                            {{ option.description }}
                          </p>
              ...
              <div class="mb-4">
                <div class="accordion-card border rounded shadow-sm" data-accordion="other">
                  <div class="accordion-header">
                    <h2 class="mb-0 text-base">
                      <button
                        type="button"
                  ...
                  <div v-show="activeAccordion === 'other'" class="accordion-content p-4">
                    <div class="space-y-2">
                      <div v-for="(option, index) in otherOptions" :key="index" class="sub-accordion border-0 rounded">
                        <div class="sub-accordion-header p-0">
                          <button
                            type="button"
                            class="sub-accordion-btn w-full text-left px-2 py-3 flex items-center justify-between hover:bg-gray-50 transition-colors relative"
                        ...
                        <div v-show="activeSubAccordion === 'other-' + index" class="sub-accordion-content p-4">
                          <p class="text-sm text-gray-600 mb-4">
                            {{ option.description }}
                          </p>
                ...
                <div class="mt-4">
                  <div class="input-file-wrapper relative">
                    ...
                    >
                      ...
                      <svg class="w-4 h-4 mt-2 ml-2" fill="currentColor" viewBox="0 0 20 20">
                        <path
                  ...
                  <div id="preview" class="preview-container mt-4 grid grid-cols-3 gap-2">
                    <div
...
const recaptchaScriptLoaded = useState<boolean>('theme-contact-recaptcha-script-loaded', () => false);
const recaptchaScriptLoading = useState<boolean>('theme-contact-recaptcha-script-loading', () => false);
const recaptchaSiteKey = computed(() => String(runtimeConfig.public.recaptchaSiteKey || '').trim());
...
const RECAPTCHA_SCRIPT_ID = 'google-recaptcha-api-script';

const getRecaptchaApi = (): RecaptchaApi | undefined => {
  const maybeWindow = window as Window & { grecaptcha?: unknown };
  const api = maybeWindow.grecaptcha;
  if (!api || typeof api !== 'object' || !('render' in api)) {
    return undefined;
  }

  return api as RecaptchaApi;
};

const renderRecaptchaWidget = () => {
  if (!import.meta.client || !recaptchaElement.value || recaptchaWidgetId.value !== null) {
    return;
  }
  ...
  recaptchaWidgetId.value = recaptchaApi.render(recaptchaElement.value, {
    sitekey: siteKey,
...
const loadRecaptchaScript = () => {
  ...
  if (getRecaptchaApi() || recaptchaScriptLoaded.value) {
    recaptchaScriptLoaded.value = true;
    renderRecaptchaWidget();
    return;
  }
  ...
  if (existingScript) {
    existingScript.addEventListener('load', handleRecaptchaScriptLoad, { once: true });
    return;
  }
  ...
  script.async = true;
  script.defer = true;
  script.addEventListener('load', handleRecaptchaScriptLoad, { once: true });
  document.head.appendChild(script);
  recaptchaScriptLoading.value = true;
};

const cleanupRecaptchaScriptListener = () => {
  if (!import.meta.client) {
    return;
  }
  ...
  const existingScript = document.getElementById(RECAPTCHA_SCRIPT_ID) as HTMLScriptElement | null;
  if (existingScript) {
    existingScript.removeEventListener('load', handleRecaptchaScriptLoad);
  }
};
...
onBeforeUnmount(() => {
  cleanupRecaptchaScriptListener();
