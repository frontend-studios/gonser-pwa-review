<template>
  ...
  >
    <div class="flex min-w-0 flex-col gap-4 md:grid md:grid-cols-[5.5rem_minmax(0,1fr)] md:items-start">
      <div
        v-if="hasMultipleImages && showThumbnails"
        class="thumbnail-list order-2 flex flex-row gap-2 max-w-[var(--preview-width)] overflow-x-auto overflow-y-hidden [scrollbar-gutter:stable] [scrollbar-width:thin] md:order-1 md:w-24 md:max-w-24 md:max-h-[var(--preview-height)] md:flex-col md:overflow-x-hidden md:overflow-y-auto md:pr-2 md:[scrollbar-width:none] md:[scrollbar-gutter:auto] md:hover:[scrollbar-width:thin] md:focus-within:[scrollbar-width:thin]"
      >
        <a
      ...
      <a
        ...
        :href="mainLightboxHref"
        :data-fancybox="resolvedGroup"
        :data-caption="currentImageCaption"
        :data-type="currentImage.type === 'image' ? undefined : currentImage.type"
        ...
        :aria-label="currentImageAlt || 'Open gallery image'"
      >
        <NuxtImg
          :src="mainDisplaySrc"
          :alt="currentImageAlt"
          :title="currentImageTitle"
          class="block aspect-square h-auto max-h-[600px] w-full object-contain"
...
const galleryItems = computed<FancyboxGalleryItem[]>(() => {
  const imageItems = props.images.reduce<FancyboxImageItem[]>((items, image) => {
    const src = productImageGetters.getImageUrl(image) || '';
    if (!src) return items;

    const displaySrc =
      productImageGetters.getImageUrlSecondPreview(image) ||
    ...
    items.push({
      ...
      src,
      displaySrc,
      thumbSrc: productImageGetters.getImageUrlPreview(image) || src,
...
const mainDisplaySrc = computed(() => {
  ...
  return currentImage.value.displaySrc || currentImage.value.thumbSrc || currentImage.value.src;
});
