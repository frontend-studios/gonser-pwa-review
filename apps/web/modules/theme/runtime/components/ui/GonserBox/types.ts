...
export interface GonserBoxProps {
  ...
  /**
   * Kompakte Darstellung (für Cart Drawer)
   * Versteckt: Bild, Buttons, zweiten Info-Text
   * @default false
   */
  compact?: boolean;
  ...
  /**
   * Info-Texte nur auf Mobile ausblenden
   * @default false
   */
  hideInfoTextsMobile?: boolean;

  /**
   * Bild nur auf Mobile ausblenden
   * @default false
   */
  ...
  /**
   * Layout-Variante
   * - 'default': Vertikales Layout (Produktseite)
   * - 'horizontal': Horizontales Layout (QuickCheckout Modal)
   * @default 'default'
   */
  layout?: 'default' | 'horizontal';

  /**
   * Content (Text+Bild) ausblenden wenn Ziel erreicht
   * Nur für Cart/CartDrawer - NICHT für Produktseiten
   * @default false
   */
  hideContentWhenReached?: boolean;

  /**
   * Manuelle Steuerung der Sichtbarkeit (überschreibt shouldShow-Logik)
   * @default undefined (verwendet interne shouldShow-Logik)
   */
  forceShow?: boolean;

  /**
   * Produkt-Objekt für Property-Check
   * Falls nicht angegeben, wird currentProduct aus useProducts() verwendet
   * @default undefined (verwendet currentProduct)
   */
  product?: any;
}
