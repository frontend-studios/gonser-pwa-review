<template>
  <div v-if="haveBadges" data-testid="badges" class="z-[2]">
    <ul class="leading-[0]">
      ...
      <!-- Availability Icon mit optionalem Text oder Tooltip -->
      <li
        v-if="useAvailability && productGetters.getAvailabilityName(product)"
        class="relative inline-flex items-center gap-2"
        :class="showAvailabilityText ? '' : 'cursor-pointer'"
        @mouseenter="!showAvailabilityText && (showTooltip = true)"
        @mouseleave="!showAvailabilityText && (showTooltip = false)"
      >
        <img :src="availabilityIcon" :alt="availabilityName" class="w-[18px] h-[18px]" />
        <span v-if="showAvailabilityText" class="text-sm text-black">
          {{ availabilityName }}
        </span>
        <div
          v-if="showTooltip && !showAvailabilityText"
...
} = defineProps<BadgesProps & { showAvailabilityText?: boolean }>();
